...
pub(crate) const V1_PATH: &str = "/api/v1/units";

pub fn configure() -> impl FnOnce(&mut ServiceConfig) {
  |config: &mut ServiceConfig| {
    config.service(web::scope(V1_PATH).service(get_unit).service(get_units));
  }
}
...
pub(crate) async fn get_unit(
  ...
  let result = web::block(move || {
    ...
    Ok::<Unit, anyhow::Error>(units::table.find(id.into_inner()).get_result::<Unit>(&mut conn)?)
  })
  .await;
...
#[utoipa::path(
  context_path = V1_PATH,
  responses(
    (status = OK, body = Vec<UnitResponse>),
    (status = 401),
    (status = 500),
  ),
  security(
...
pub(crate) async fn get_units(
  ...
  let result = web::block(move || {
    ...
    Ok::<Vec<Unit>, anyhow::Error>(units::table.load::<Unit>(&mut conn)?)
  })
  .await;
