...
import { Select, Button, ActionIcon, Group, Paper, Text, Divider, Badge, NumberInput } from "@mantine/core"
...
export function GroceryList() {
  ...
  useEffect(() => {
    const total = groceryItems.reduce((sum, item) => sum + item.price * item.quantity, 0)
    setTotalCost(total)
  }, [groceryItems])
  ...
  const handleAddItem = () => {
    ...
    if (selectedStore !== product.at && product.otherPrices && product.otherPrices[selectedStore]) {
  ...
  const handleUpdateQuantity = (id: string, newQuantity: number) => {
    ...
    setGroceryItems(groceryItems.map((item) => (item.id === id ? { ...item, quantity: newQuantity } : item)))
  }
  ...
  const handleChangeStore = (id: string, newStore: string) => {
    setGroceryItems(
      groceryItems.map((item) => {
        ...
        if (newStore !== product.at && product.otherPrices && product.otherPrices[newStore]) {
          newPrice = product.otherPrices[newStore].price || newPrice
          newUnitPrice = product.otherPrices[newStore].weightPrice || newUnitPrice
        }
      ...
      <Paper shadow="xs" p="md" withBorder className="space-y-4">
        {groceryItems.length === 0 ? (
          ...
          <>
            {groceryItems.map((item) => (
              <div key={item.id}>
                <Group position="apart" className="flex items-center">
                  <Group className="flex-grow">
                    <NumberInput
                      value={item.quantity}
                      onChange={(value) => handleUpdateQuantity(item.id, Number(value) || 1)}
                      min={1}
                      className="w-16"
                    />
                  ...
                  <Group>
                    <Select
                      data={storeOptions}
                      value={item.store}
                      onChange={(value) => handleChangeStore(item.id, value || "Walmart")}
                      className="w-32"
                    />
